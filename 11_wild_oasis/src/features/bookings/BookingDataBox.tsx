...
import { SingleBooking } from "../../types";
...
function BookingDataBox({ booking }: BookingDataBoxProps) {
  ...
  const country_flag = guests?.country_flag;
    ...
    <StyledBookingDataBox>
      ...
      <Section>
        <Guest>
          ...
          <p>
            {guestName}{" "}
            {(num_guests || 0) > 1 ? `+ ${(num_guests || 0) - 1} guests` : ""}
          </p>
          <span>&bull;</span>
          <p>{email}</p>
          <span>&bull;</span>
        ...
        <Price $isPaid={is_paid || false}>
          <DataItem icon={<HiOutlineCurrencyDollar />} label={`Total price`}>
            {formatCurrency(total_price || 0)}

            {has_breakfast &&
              ` (${formatCurrency(cabin_price || 0)} cabin + ${formatCurrency(
                extras_price || 0,
              )} breakfast)`}
          </DataItem>
